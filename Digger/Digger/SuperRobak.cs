...
namespace Digger
{
	...
	class SuperRobak:Characters
	{
        ...
        public SuperRobak(Texture2D newTexture, Vector2 newPosition,Tony newTony)
           ...
           // int x = 0, y = 0;
            ...
            if (position.X - oTony.position.X < 0)
            {
                ...
                if (position.X + 24 < 816)
                    if (Map.map[x , y].isEmpty == StateField.fill)
                        Map.map[x , y].isEmpty = StateField.empty;
                position.X += speed;
            }
            ...
            if (position.Y - oTony.position.Y < 0 )
            {
                if (x >= 16) x = 16;
                if (y >= 11) y = 11;
            ...
            else if (position.Y - oTony.position.Y > 0 )
            {
                if (x >= 16) x = 16;
                if (y >= 11) y = 11;
                if (position.Y - 24 > 48 && Map.map[x, y - 1].isEmpty == StateField.empty)
                    if (Map.map[x , y].isEmpty == StateField.fill)
                        Map.map[x , y].isEmpty = StateField.empty;
                position.Y -= speed;
            }
